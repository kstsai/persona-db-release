...
UP = pathlib.Path("meta/original-upstream-f57392bb.sh")
OUT = pathlib.Path("run-test.sh")

src = UP.read_text(encoding="utf-8")
lines = src.split("\n")
assert lines[0].startswith("#!/bin/bash"), "upstream 開頭不是 shebang"
assert len(lines) == 373, f"upstream 應為 372 行（+尾端空行=373 元素），實得 {len(lines)}"
...
assert len(part_c) == 372 - 87 + 1, f"part_c 應為 286 元素（L88–L372 + 尾端空行），實得 {len(part_c)}"
...
# ── OUT 預設值由「本目錄名」推導，不寫死（2026-09-15 踩過的坑）──
# 我複製第九輪的工具來開第十輪時忘了改 runner 內寫死的 `$HOME/qa-round9`，
# 結果第十輪的執行**寫進第九輪的目錄**，就地覆寫了已發布的證據。
# 改成從目錄名推導後，這種事不可能再發生。
ROUND_TAG = pathlib.Path(".").resolve().name.split("-")[0]      # 例：round10
OUT_DEFAULT = f"$HOME/qa-{ROUND_TAG}"                           # 例：$HOME/qa-round10

CASES = [
    ("01_kangshimei",     "kangshimei",         ["questions=康是美的目標客戶", "top_k=3", "opMode=僅篩選"]),
    ("02_tesla",          "tesla",              ["questions=TESLA的目標客戶", "top_k=3", "opMode=僅篩選"]),
    ("03_fashion",        None,                 ["questions=時尚服裝設計師的目標客戶", "top_k=10", "opMode=僅篩選"]),
...
case_run() {{   # case_run <id> <label 原文> <tmpname|-> <curl args...>
  ...
  if [ "$tmp" != "-" ]; then
    ...
    # upstream 此案例為 `| jq > fashion_closing.json`：保留 jq 美化輸出，
    # 但落盤到證據目錄（原本寫在 CWD；無任何斷言讀取該檔）
    jq . "$OUT/raw/${{id}}.body" | tee "$OUT/json/${{id}}.json"
  fi
  echo ""
...
print(f"   part_c = upstream L88–L372（{len(part_c)} 元素，逐字）")
print(f"   案例標籤 = 直接取自 upstream 原文（{len(UP_LABELS)} 個）")
assert OUT_DEFAULT in body, f"❌ runner 內找不到推導出的 OUT 預設值 {OUT_DEFAULT}"
assert "qa-round9" not in body, "❌ runner 內殘留上一輪的目錄名 qa-round9"
assert "$HOME/qa-round9" not in body, "❌ runner 內殘留上一輪的目錄名"
print(f"   OUT 預設 = {OUT_DEFAULT}（由目錄名 '{ROUND_TAG}' 推導，並已確認無上一輪殘留）")
