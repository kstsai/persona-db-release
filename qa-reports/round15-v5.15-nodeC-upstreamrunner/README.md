# round15 — Persona DB v5.15 / NODE-C / upstream runner

**單版本 API 驗證與回應分析**（依 `api-version-sweep` skill **v2.8.0**）。
**本輪不做跨版本比對** —— 只分析 v5.15 這個版本的回應。

- 主報告：**[ANALYSIS.md](ANALYSIS.md)** ← 先讀這個
- **⚠️ 誤判與更正（給其他 agent）**：**[FALSE-POSITIVE-NODE-C-ssh.md](FALSE-POSITIVE-NODE-C-ssh.md)** ← 初稿「NODE-C 無 SSH 存取」是 **false positive**，附根因與可複用教訓
