...
try:
    import urllib.request as _u56, os
    _os56 = json.load(_u56.urlopen(os.environ.get("BASE_URL", "http://localhost:8000") + "/openapi.json", timeout=10))
...
try:
    import urllib.request as _u, os
    _os = json.load(_u.urlopen(os.environ.get("BASE_URL", "http://localhost:8000") + "/openapi.json", timeout=10))
...
try:
    import urllib.request as _u, os
    _spec = json.loads(_u.urlopen(os.environ.get("BASE_URL", "http://localhost:8000") + "/openapi.json", timeout=10).read())
...
# ── #60/#55/#50：主機層（docker）斷言 ──
# ⚠️ 本輪 runner 以遠端執行（runner 內無 docker 存取）→ 這些主機層斷言在 runner 內標 N/A；
#    已另行以節點存取（ubuntu@NODE-C）補跑 #50/#55/#60 → 見 extra/deployment-fidelity.txt。
#    部署保真度（執行中 code == v5.15 tarball）無法從節點內驗證，列為本輪限制。
echo "  --- v5.10 (#60 root logger) ---"
echo "  #60 app INFO 行是否進 log → N/A（遠端執行，無 docker logs 存取）"
echo "  #60 容器 LOG_LEVEL → N/A（遠端執行，無 docker exec 存取）"
echo "  #55 映像含例外型別診斷碼 → N/A（遠端執行，無 docker exec 存取）"
echo "  --- v5.6 (#50) / 部署版本一致性 ---"
echo "  部署版本 == RELEASE-VERSION → N/A（遠端執行，無 docker exec 存取）"
echo "  #50 映像含 finish_reason 診斷碼 → N/A（遠端執行，無 docker exec 存取）"
