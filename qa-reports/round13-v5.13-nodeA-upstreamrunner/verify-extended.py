...
def main():
    ...
    for cid, top_k in TOP_K.items():
        ...
        # ── #66 / #67（v5.13 新增）──
        srcs = d.get("protected_dims_sources") or {}
        sb = d.get("subject_basis", None)
        if srcs:
            rec("Y1", set(srcs) == {"domain","reasoning","analysis_declared","model_declared","protect_only"},
                f"#66 A 來源五鍵齊全（實際={sorted(srcs)}）")
