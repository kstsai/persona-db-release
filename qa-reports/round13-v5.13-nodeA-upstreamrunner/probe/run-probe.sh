...
#
#   t2 用的是**刻意收窄的醫美查詢**，目的是逼出 v5.13 新增的
#       stop_reason='protection_saturated'（#66 C：保護集覆蓋全部已套用維度 → 停手）。
#       要觸發它需要「matched < TARGET_MIN(20)」且「所有已套用維度都在保護集內」。
#       醫美題的保護集通常只有 aesthetic_procedure，若模型也只套這一個維度就會立即飽和。
#       出貨斷言自己註明「實際觸發需全維度受保護，套件通常不會出現」→ 所以必須定向逼它。
...
# ── §6.8 重現性探針 ──────────────────────────────────────────────────
# case08（小吃攤老闆的目標客群 = 顧客語意）：第十二輪量到 #65 主體護欄 4/4 生效；
#   本輪續看它是否仍穩定、以及新增的 subject_basis（#67）是否每次都有依據。
for i in 1 2 3; do
  probe "r08_boss_${i}" "§6.8 重現性 — case08 小吃攤（顧客語意）r${i}/3（#65/#67）" \
    --data-urlencode "questions=小吃攤老闆的目標客群" \
    --data-urlencode "role=夜市商圈協會" \
    --data-urlencode "top_k=10" \
    --data-urlencode "opMode=僅篩選"
done
# case06（醫美）：核心保護 + 值集放寬留痕的穩定度；也用來看是否有機會自然觸發
#   protection_saturated（醫美常只套 aesthetic_procedure）。
for i in 1 2 3; do
...
probe "t1_narrow_topk10" "§6.9 t1 不可滿足查詢 top_k=10（題目原文見檔頭註解）" \
  ...
  --data-urlencode "top_k=10" \
  --data-urlencode "opMode=僅篩選"
# t2：刻意收窄的醫美查詢 → 逼出 protection_saturated（#66 C）
probe "t2_aesthetic_narrow" "§6.9 t2 收窄醫美查詢 → 逼 protection_saturated（#66 C）" \
  --data-urlencode "questions=想找做過醫美療程的45歲單身女性" \
  --data-urlencode "top_k=10" \
  --data-urlencode "opMode=僅篩選"
# t3：TESLA + 較大 top_k → #61 commute_mode 保護
probe "t3_tesla_topk20" "§6.9 t3 TESLA top_k=20 → #61 commute_mode 保護" \
  --data-urlencode "questions=TESLA的目標客戶" \
  --data-urlencode "top_k=20" \
  --data-urlencode "opMode=僅篩選"
# t4：房貸（銀行）+ 較大 top_k → 值集放寬 / veto
