...
def main():
    ...
    # ── 總表 ───────────────────────────────────────────────────────────
    hr("§2 執行摘要（機械抽出）")
    hdr = f"{'case':20s} {'matched':>8s} {'ret':>4s} {'sum':>4s} {'pe':>5s} {'loops':>6s} {'noop':>5s} {'ovs':>4s}  stop_reason"
    print(hdr)
    print("-" * len(hdr))
    for cid in ok_cases:
        d = cases[cid]
        ...
        print(f"{cid:20s} {d.get('total_matched', -1):8d} {d.get('returned', -1):4d} "
              ...
              f"{d.get('broadening_stop_reason')!r}")
    ...
    for cid in ok_cases:
        ...
        for b in ba:
            print(f"      loop{b.get('loop')}: {b.get('match_count_before')}→{b.get('match_count_after')} "
                  ...
                  f"changed={b.get('filters_changed')} change={str(b.get('change'))[:150]!r}")
