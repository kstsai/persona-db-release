...
```bash
sha256sum raw/openapi.json ../personadb-dh1-api-verify/raw/openapi.json
# 兩者應相同 → 契約未變 → 版本差異只能來自行為或 LLM 雜訊
diff <(cat raw/00_status.body) <(cat ../personadb-dh1-api-verify/raw/00_status.body)
...
python3 - <<'PY'
import json,glob,os
for lab,d in (('v5.3.1','.'),('v5.2','../personadb-dh1-api-verify')):
    ms=[]
...
PY
cat ../personadb-dh1-api-verify/repeat/r3-timeout.txt   # v5.2 的第 4 次為 300s 逾時
