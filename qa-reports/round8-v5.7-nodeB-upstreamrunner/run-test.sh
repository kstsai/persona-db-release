#!/bin/bash
# Round 8 runner — adapted from upstream `upDockerVerHermes/test-persona-db-api.sh`
#   upstream sha256: 5c1ca7165049f449745f43813edc01190fa25d171fa8f2228ceef71010bcf537  (227 行, 2026-09-14 取得)
#
# 與上一輪（上游 9a29a8ef, 223 行）的**唯一差異**：`#42` 精度斷言由絕對門檻
#   `len(have) >= 9` 改為比例式 `len(s)>0 and len(have)/len(s) >= 0.9`
#   —— 即 issue #52 的修復（觸發自第七輪 §2 的發現，由 hermesa3 於 4a9071c 完成）。
#   本 runner 已同步該段（與新上游逐字相同，可用 diff 驗證）。
#
# 沿用既有 baseline（上游 223 行版）的其餘全部內容：案例 1–9（含 #34 業主語意案例）。
#
