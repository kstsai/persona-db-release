...
| `NODE-B-host` | `NODE-B` 的節點內部 HostName（與 私有網路 名不同）|
| `NODE-C` | **第十五輪**的受測節點（`NODE-C` 為唯一**遠端執行**的輪次；以 `ubuntu` 帳號＋**密碼認證**存取，非 OpenSSH key）|
| `[public-ip]` / `[ts-ipv6]` / `[peer-ip]` | 已遮蔽的位址 |
| `[private-net]` | 私有網路 DNS 後綴 |
| `[account]` | 私有網路 帳號 |
| `[peer]` | 其他 fleet 成員（僅出現於 `私有 mesh VPN status` 傾印）|
...
- **去識別化只涵蓋現行檔案，不含 git 歷史**：`git log -p` 仍可取得遮蔽前的原始值。
  **✅ 已決策（2026-09-14，kstsai）：接受此狀態，不改寫歷史。**
  ...
  （遮蔽目標主要為 私有 mesh VPN CGNAT 位址，需 私有網路 成員身分才有意義）。
