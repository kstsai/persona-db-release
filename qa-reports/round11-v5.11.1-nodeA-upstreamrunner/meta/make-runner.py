...
UP = pathlib.Path("meta/original-upstream-f3cb96ad.sh")
OUT = pathlib.Path("run-test.sh")

src = UP.read_text(encoding="utf-8")
lines = src.split("\n")
assert lines[0].startswith("#!/bin/bash"), "upstream 開頭不是 shebang"
assert len(lines) == 422, f"upstream 應為 421 行（+尾端空行=422 元素），實得 {len(lines)}"
...
assert len(part_c) == 421 - 87 + 1, f"part_c 應為 335 元素（L88–L421 + 尾端空行），實得 {len(part_c)}"
...
print(f"   part_c = upstream L88–L421（{len(part_c)} 元素，逐字）")
